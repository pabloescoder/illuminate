...
class RegisterAPI(generics.GenericAPIView):
    ...
    def post(self, request):
        user = request.data
        if NewUser.objects.filter(email=user["email"]).filter(is_verified=False).exists():
            NewUser.objects.filter(email=user["email"]).delete()
        else:
            if NewUser.objects.filter(email=user["email"]).filter(is_verified=True).exists():
                return Response(
...
class UserProfileView(RetrieveUpdateAPIView):
    ...
    def get(self,request):
        try:
            return JsonResponse(self.serializer_class(self.queryset.get(id=request.user.id)).data)
        except Exception as e:
            return Response(status=status.HTTP_400_BAD_REQUEST)